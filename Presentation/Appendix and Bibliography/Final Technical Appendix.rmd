...
```{r Load and, if necessary, install the package first, echo=TRUE, message=FALSE}
...
```{r Tidying the Data, echo=TRUE, message=FALSE}
...
part_tu04_sex_TRCHILDNUM_plot <- ggplot(part_sex_weighted_means_data, aes(x=TUYEAR, y=part_tu04)) +
  ...
  geom_smooth(method = "lm", data=part_sex_weighted_means_TRCHILDNUM, aes(x=as.numeric(as.character(TUYEAR)), y=TRCHILDNUM, col=TESEX), se=FALSE, linetype="dashed", size=1) +
  ...
  scale_x_continuous(breaks = seq(2003,2017,2)) + xlab("Year") + ylab("Average number of children") + labs(title = "Hypothesised parallels between no. children and time spent caring for household members", caption = "Data Source: ATUS Survey", color="Sex", fill="Sex") + theme(plot.title = element_text(size=9), axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))
```
...
part_tu04_sex_validation_predict <- predict(part_tu04_sex_model_validation, type = "response")
part_tu04_sex_validation_table <- as_tibble(data.frame(Year = 2003:2017,
                                            Sex = c(rep("M", 15), rep("F", 15)),
                                            Observed = part_sex_weighted_means_data_validation$part_tu04,
                                            Fitted = part_tu04_sex_validation_predict)
                                            )
part_tu04_sex_validation_table <- mutate(part_tu04_sex_validation_table, Residual = (Observed - Fitted)/sqrt(Fitted^2))
ggplot(part_tu04_sex_validation_table, aes(x=Fitted, y=Residual)) + geom_point() + ggtitle("Residual Plot for Validation Data")
```
...
part_tu04_sex_TRCHILDNUM_plot_plotting <- ggplot(part_sex_weighted_means_data_plotting, aes(x=TUYEAR, y=part_tu04)) +
  ...
  geom_smooth(method = "lm", data=part_sex_weighted_means_TRCHILDNUM_plotting, aes(x=as.numeric(as.character(TUYEAR)), y=TRCHILDNUM, col=TESEX), se=FALSE, linetype="dashed", size=1) +
  ...
  scale_y_continuous(sec.axis = sec_axis(~.*(1/0.1), name = "Spent any time at all caring for & \n helping nonhousehold members (%) \n")) +
  scale_x_continuous(breaks = seq(2003,2017,2)) + xlab("Year") + ylab("Average number of children") + labs(color="Sex", fill="Sex", caption = "Data Source: ATUS Survey", title = "Parallels between number of children and time spent caring for household members") + theme(plot.title = element_text(size=9), axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))
```
...
```{r Subset and generate features}
...
Note that `region` is appended but was not used extensively in the final report. It was an area of interest for EDA but more interesting hypotheses could be formed using `generation` and so this was chosen instead.
...
The code for the two tables produced in the EDA section is below.

```{r}
Male.Activities <- c("Working", "House Maintenance", "Vehicle Maintenance")
Female.Activities <- c("Housework", "Cooking", "Childcare")
kable(data.frame(Male.Activities, Female.Activities), caption = "Traditionalist Gender Actitivies", format = "latex", booktabs = T, col.names = c("Male Activites", "Female Activities"), align = c('l', 'r')) %>% kable_styling(latex_options = c("hold_position"))
...
kable(data.frame(Generations,Birth.Years),format = "latex", caption = "Generations", booktabs = T, col.names = c("Generation", "Birth Years"), align = c('l', 'r')) %>% kable_styling(latex_options = c("hold_position"))
```
...
fit.models.and.plot <- function(Data, variable, mu) {
    ...
    if (variable == 'Working' | variable == 'Childcare') {
        ...
        geom_line() +
        geom_ribbon(aes(ymin = prediction - prediction_error, ymax = prediction + prediction_error), alpha = 0.2, linetype = 0) + facet_wrap(~generation, nrow = 1) + labs(x = "Year", y = paste("Predicted",variable,"(minutes)"), title = paste("Trends in", tolower(gsub("[.]", " ", variable)), "for each generation"), caption = "Data Source: ATUS Survey"))
    } else {
...
```{r Gender difference data frame generation}
...
The following plots show the low participation rates for Vehicle Maintenance and House Maintenance as noted in the report.

```{r Low participation evidence}
