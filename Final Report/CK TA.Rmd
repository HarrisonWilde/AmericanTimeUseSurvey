...
weighted.valuesM <- c()
weighted.valuesF <- c()
Gender.Roles.Differences <- data.frame('Year' = c(), 'Month' = c(), 'Variable' = c(), 'Generation' = c(), 'Region' = c(), 'Difference' = c())
            ...
            for (year in unique(Gender.Roles.Data$TUYEAR)) {
                for (month in unique(Gender.Roles.Data$TUMONTH)) {
                    for (sex in c("M", "F")) {
                        ...
                        if (Filtered.Data$TESEX[1] == "F") {
                            weighted.difference <- Gender.Roles.Data[Gender.Roles.Data$TESEX == 'M' & Gender.Roles.Data$TUYEAR == year & Gender.Roles.Data$TUMONTH == month & Gender.Roles.Data$GEREG == region & Gender.Roles.Data$TUGENERATION == generation, paste0('weighted.', variable)] - weighted.values
                        }
                    }
                    Gender.Roles.Differences <- rbind(Gender.Roles.Differences, data.frame('Year' = year, 'Month' = month, 'Variable' = variable, 'Generation' = generation, 'Region' = region, 'Difference' = weighted.difference))
                }
            }
        }
    }
