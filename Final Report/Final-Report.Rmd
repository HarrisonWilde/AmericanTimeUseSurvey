...
We have now verified that the conlusions are correct: the percentage of American's who partake in caring for & helping nonhousehold members has decreased over the period from 2003 to 2017. There also seems to be a link between the number of children in the household and the participation percentage.
...
Following some initial exploration of the data, it was also decided that looking at different generations [@generations] presented a different look at the activities, with each generations spending different lengths of time on activities. Essentially, this provides a split in the data by age, but using generations provides a way of tracking a population over time and without having to arbitrarily pick age categories. These generations are defined in the table below. A small number of participants fell outside of these ranges, however, these represented only a small number of participants and so were added to the next nearest generation (e.g. if a participant was born in 1998, they would be added to the *Millennials* generation).
