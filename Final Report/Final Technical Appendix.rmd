...
```{r Load and, if neccessary, install the package first, echo=TRUE, message=FALSE}
...
```{r Weighted Means, echo=TRUE, message=FALSE}
...
```{r Initial Summary Data, echo=TRUE, message=FALSE}
...
```{r Initial Summary Plots, echo=TRUE, message=FALSE}
...
```{r Printing Summary Plots, eval=FALSE, echo=TRUE, message=FALSE}
...
```{r Weighted Means Variance and Percentage Change, echo=TRUE}
...
```{r Weighted Means Variance and Percentage Change Table, echo=TRUE}
kable_names <- "Measure"
for (i in 1:9){
...
```{r part_tu04 Summary Model, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r part_tu04 Summary Plot, message=FALSE, eval=FALSE, echo=TRUE}
...
```{r Sex Weighted Means, echo=TRUE, message=FALSE}
...
```{r part_tu04 Sex Model, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r part_tu04 Sex Plot, message=FALSE, eval=FALSE, echo=TRUE}
...
```{r TRCHILDNUM Weighted Means, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r Suitability of the model, echo=TRUE}
...
```{r part_tu04 Sex Plot with Number of Children, message=FALSE, echo=TRUE, fig.height=4}
...
```{r Linear model for the plot, eval=TRUE, echo=TRUE, message=FALSE}
...
```{r Adding TRCHILDNUM to model, echo=TRUE, message=FALSE}
...
```{r Testing the new model, eval=TRUE, echo=TRUE, message=FALSE}
...
```{r Validation Data Tidying, echo=TRUE, message=FALSE}
...
```{r Sex Weighted Means Validation, echo=TRUE, message=FALSE}
...
```{r Linear model for the plot Validation, eval=TRUE, echo=TRUE, message=FALSE}
...
```{r part_tu04 Sex Model Validation, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r Adding TRCHILDNUM to model Validation, echo=TRUE, message=FALSE}
...
The t-test is completed on the model and confirms the belief that there is a link is true.

```{r Testing the new model Validation, eval=TRUE, echo=TRUE, message=FALSE, fig.height=4}
...
To plot the results, all of the data, exluding those for July, is used. The same method of tidying the data, calculating weighted means, building a spline curve glm model with log link and multiplicative error and plotting a graphical interpretation of the model is performed.

```{r Plotting Data Tidying, echo=TRUE, message=FALSE}
#remove full time vs part time as has -1
...
```{r Sex Weighted Means Plotting, echo=TRUE, message=FALSE}
...
```{r part_tu04 Sex Model Plotting, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r TRCHILDNUM Weighted Means Plotting, message=FALSE, echo=TRUE, warning=FALSE}
...
```{r part_tu04 Sex Plot with Number of Children Plotting, message=FALSE, echo=TRUE, eval=FALSE}
...
for (variable in c("Working", "House.Maintenance", "Vehicle.Maintenance", "Housework", "Food.Preparation", "Childcare")) {
    for (year in unique(Gender.Roles.Validate$TUYEAR)) {
        for (sex in c("M", "F")) {
            Filtered.Data <- filter(Gender.Roles.Validate, TESEX == sex, TUYEAR == year)
            bottom.sum <- sum(Filtered.Data$TUFNWGTP)
            top.sum <- sum(Filtered.Data$TUFNWGTP * Filtered.Data[,variable])
            weighted.values <- top.sum / bottom.sum
            if (sex == 'M') m.weighted.values <- weighted.values
