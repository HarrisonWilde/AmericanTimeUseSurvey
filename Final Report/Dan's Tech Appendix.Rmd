...
ATUS_Respondent_file <- import("../Data/atusresp_0317.csv", setclass = "tibble") # import respondent data as TIBBLE
```
...
# Remove July for validation purposes
    ...
    for (generation in c("Silent Generation", "Baby Boomers", "Generation X", "Millenials")) {
        for (year in unique(Gender.Roles.Data$TUYEAR)) {
            for (month in unique(Gender.Roles.Data$TUMONTH)) {
                for (sex in c("M", "F")) {
                    Filtered.Data <- filter(Gender.Roles.Data, TESEX == sex, TUYEAR == year, TUMONTH == month, TUGENERATION == generation)
