...
ATUS_dataset <- import("../Data/atussum_0317.csv", setclass = "tibble")    # import raw summary data as TIBBLE
...
ATUS_dataset$TUDIARYDATE <- as.Date(paste0(substr(ATUS.dataset$TUCASEID, 1, 6), '01'), "%Y%m%d")
...
# Remove odd months (including July) for validation purposes
...
#remove full time vs part time as has -1 (66499 times)
Data_minus_columns <- select(ATUS_training_data, -one_of("TRDPFTPT"))
#remove full time vs part time spouse as has -1 (112134 times)
Data_minus_columns <- select(Data_minus_columns, -one_of("TRSPFTPT"))
#remove age of youngest child as has -1 (95663 times)
Data_minus_columns <- select(Data_minus_columns, -one_of("TRYHHCHILD"))
#remove 2 jobs within last 7 days as has -1 (66499 times)
Data_minus_columns <- select(Data_minus_columns, -one_of("TEMJOT"))
#remove enrolled at high school etc as has -1 and -3 and also question about which (76179 and 158595 times)
Data_minus_columns <- select(Data_minus_columns, -one_of(c("TESCHENR", "TESCHLVL")))
#remove employment status of spouse as has -1 (82506 times)
Data_minus_columns <- select(Data_minus_columns, -one_of("TESPEMPNOT"))
#remove weekly earnings as has -1 (78897 times)
...
sex_model_D_squared <- Dsquared(part_tu04_sex_model)
...
The continuous activity data is non-negative and non-linear. Therefore a suitable model, using **Year** and **Sex** as explanatory variables, is a natural cubic spline GLM with multiplicative errors and log link. In fact it gives a D-squared accuracy score $\approx$ `r round(sex_model_D_squared, 2)`. An F test shows this model is a significant improvement over the same model which does not use **Sex** as a variable, confirming the belief that is has  a significant effect on participation in caring for & helping nonhousehold members. 
...
```{r, echo=FALSE}
validation_part_tu04_predicted <- predict(part_tu04_sex_model, newdata = part_sex_weighted_means_data_validation, type = "response")
validation_part_tu04_actual <- part_sex_weighted_means_data_validation$part_tu04
#sex_model_validation_D_squared <- Dsquared(obs=validation_part_tu04_actual, pred=validation_part_tu04_predicted)
```

The first step to test the suitability of the original model is to test its accuracy at predicting the proportion of American's who participate in caring for & helping non-household members for the validation data. The model gives an D-squared accuracy score of ` round(sex_model_validation_D_squared)`. Methods to tune parameters for models weren't used for this analysis and therefore it is possible that the model was over-fitting the training data. However, a score of ` round(sex_model_validation_D_squared)` is sufficient to continue with the validation as it appears that the trend is the same as in the EDA.
...
```{r TRCHILDNUM Weighted Means Validation, message=FALSE, echo=FALSE, warning=FALSE}
sex_weighted_means_TRCHILDNUM_validation <- as_tibble(expand.grid(TUYEAR=seq(2003, 2017), TESEX=as.factor(c("M", "F"))))
for (i in 1:9){
  sex_weighted_means_TRCHILDNUM_validation <- full_join(sex_weighted_means_TRCHILDNUM_validation, summarise_at(Data_minus_columns_year_sex_grouped_validation, vars(TRCHILDNUM), funs(weighted.mean(., TUFNWGTP))))
}
for (i in c(10:16, 18)){
  sex_weighted_means_TRCHILDNUM_validation <- full_join(sex_weighted_means_TRCHILDNUM_validation, summarise_at(Data_minus_columns_year_sex_grouped_validation, vars(TRCHILDNUM), funs(weighted.mean(., TUFNWGTP))))
}

part_sex_weighted_means_TRCHILDNUM_validation <- as_tibble(expand.grid(TUYEAR=seq(2003, 2017), TESEX=as.factor(c("M", "F"))))
for (i in 1:9){
  part_sex_weighted_means_TRCHILDNUM_validation <- full_join(part_sex_weighted_means_TRCHILDNUM_validation, summarise_at(Data_minus_columns_year_sex_grouped_validation, vars(TRCHILDNUM), funs(weighted.mean(., TUFNWGTP))))
}
for (i in c(10:16, 18)){
  part_sex_weighted_means_TRCHILDNUM_validation <- full_join(part_sex_weighted_means_TRCHILDNUM_validation, summarise_at(Data_minus_columns_year_sex_grouped_validation, vars(TRCHILDNUM), funs(weighted.mean(., TUFNWGTP))))
}

part_tu04_sex_TRCHILDNUM_plot_validation <- ggplot(part_sex_weighted_means_data_validation, aes(x=TUYEAR, y=part_tu04)) +
  geom_bar(data=part_sex_weighted_means_TRCHILDNUM_validation, aes(x=as.numeric(as.character(TUYEAR)), y=TRCHILDNUM, fill=TESEX), stat="identity", position = "dodge", width = 0.8, alpha = 0.3) + 
  geom_smooth(method = "lm", data=part_sex_weighted_means_TRCHILDNUM_validation, aes(x=as.numeric(as.character(TUYEAR)), y=TRCHILDNUM, col=TESEX), se=FALSE, linetype="dashed", size=1.5) +
  geom_line(data = part_tu04_sex_predicted_validation, aes(x=TUYEAR, y=0.1*fitted, col=TESEX), size=1.3) +
  geom_ribbon(data = filter(part_tu04_sex_predicted_validation, TESEX =="M"), aes(y=0.1*fitted, ymin=0.1*ymin, ymax=0.1*ymax),alpha=0.1) +
  geom_ribbon(data = filter(part_tu04_sex_predicted_validation, TESEX =="F"), aes(y=0.1*fitted, ymin=0.1*ymin, ymax=0.1*ymax),alpha=0.1) +
  scale_y_continuous(sec.axis = sec_axis(~.*(1/0.1), name = "Spent any time at all caring for & \n helping nonhousehold members (%) \n")) + 
  scale_x_continuous(breaks = seq(2003,2017,2)) + xlab("Year") + ylab("Average number of children") + labs(color="Sex", fill="Sex") + theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))
```

A new model of the same structure (natural spline with log link and multiplicative error) was built It fit the validation data with a D-squared accuracy score of $\approx$ `r round(Dsquared(part_tu04_sex_model_validation))`. A graphical interpretation of this can be seen below.

```{r part_tu04 Sex Plot with Number of Children Validation, message=FALSE, echo=FALSE}
print(part_tu04_sex_TRCHILDNUM_plot_validation)
```

Aswell as the fact participation in caring for & helping nonhousehold members has clearly gone down over the period, it is also again appears that the initial thoughts about the link with number of household children, from the EDA, are correct. This can be formally checked with a t-test on an updated model which includes the number of household children. 
...
Gender.Roles.Data <- select(ATUS_dataset, TUCASEID, TESEX, TEAGE, TUFNWGTP, TUYEAR, TRCHILDNUM, TUDIARYDATE, TUGENERATION)
    ...
    for (generation in c("Silent Generation", "Baby Boomers", "Generation X", "Millennials")) {
        for (year in unique(Gender.Roles.Data$TUYEAR)) {
            for (month in unique(month(Gender.Roles.Data$TUDIARYDATE))) {
                for (sex in c("M", "F")) {
                    Filtered.Data <- filter(Gender.Roles.Data, TESEX == sex, TUYEAR == year, month(TUDAIRYDATE) == month, TUGENERATION == generation)
...
for (variable in c("Working", "House.Maintenance", "Vehicle.Maintenance", "Housework", "Food.Preparation", "Childcare")) {
    ...
    # model1 <- glm(as.formula(paste0('weighted.',variable,' ~ ns(TUDIARYDATE, knots = (c(as.Date("2006-01-01"), as.Date("2009-01-01"), as.Date("2012-01-01"), as.Date("2015-01-01"))))')), data = Gender.Roles.Validate, family = quasi(link = "log", variance = "mu^2"), mustart = rep(10, n))
    model1 <- glm(as.formula(paste0('weighted.',variable,' ~ ns(TUDIARYDATE)')), data = Gender.Roles.Validate, family = quasi(link = "log", variance = "mu^2"), mustart = rep(10, n))
    model2 <- update(model1, . ~ . + TESEX)
    # model3 <- update(model1, . ~ -1 + TESEX + TESEX:ns(TUDIARYDATE, knots = (c(as.Date('2006-01-01'), as.Date('2009-01-01'), as.Date('2012-01-01'), as.Date('2015-01-01')))))
    model3 <- update(model1, . ~ -1 + TESEX + TESEX:ns(TUDIARYDATE, knots = c(as.Date(Dates))))
    model4 <- update(model3, .~. + GEREG + GEREG:ns(TUDIARYDATE, knots = c(as.Date(Dates))))
    ...
    # Use same method as in lab 5 to plot them
    # change the model number in the predict functions below for different models
    male.prediction <- predict.glm(model5, newdata = filter(Gender.Roles.Validate, TESEX == 'M'), se = TRUE, type = "response")
    female.prediction <- predict.glm(model5, newdata = filter(Gender.Roles.Validate, TESEX == 'F'), se = TRUE, type = "response")
...
for (variable in c("Working", "House.Maintenance", "Vehicle.Maintenance", "Housework", "Food.Preparation", "Childcare")) {
    ...
    # model1 <- glm(as.formula(paste0('weighted.',variable,' ~ ns(TUDIARYDATE, knots = (c(as.Date("2006-01-01"), as.Date("2009-01-01"), as.Date("2012-01-01"), as.Date("2015-01-01"))))')), data = Gender.Roles.Data, family = quasi(link = "log", variance = "mu^2"), mustart = rep(10, n))
    model1 <- glm(as.formula(paste0('weighted.',variable,' ~ ns(TUDIARYDATE)')), data = Gender.Roles.Data, family = quasi(link = "log", variance = "mu^2"), mustart = rep(10, n))
    model2 <- update(model1, . ~ . + TESEX)
    # model3 <- update(model1, . ~ -1 + TESEX + TESEX:ns(TUDIARYDATE, knots = (c(as.Date('2006-01-01'), as.Date('2009-01-01'), as.Date('2012-01-01'), as.Date('2015-01-01')))))
    model3 <- update(model1, . ~ -1 + TESEX + TESEX:ns(TUDIARYDATE, knots = c(as.Date(Dates))))
    model4 <- update(model3, .~. + GEREG + GEREG:ns(TUDIARYDATE, knots = c(as.Date(Dates))))
