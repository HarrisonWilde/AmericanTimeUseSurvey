...
We have now verified that the conclusions suggested by the EDA are correct: the percentage of American's who partake in caring for & helping non-household members has decreased over the period from 2003 to 2017. There also seems to be a link between the number of children in the household and the participation percentage.  The plot below gives a graphical interpretation of changes using a model fit on all the data, only excluding those responses taken in July for each year.
...
Female.Activities <- c("Housework", "Food Preparation", "Childcare")
kable(data.frame(Male.Activities, Female.Activities), caption  = "Traditionally Perceived Gender Activities", format = "latex", booktabs = T,  col.names = c("Male Activites", "Female Activities"), align = c('l', 'r')) %>% kable_styling(latex_options = "HOLD_position")
```
...
kable(data.frame(Generations,Birth.Years),format = "latex", caption = "Generations", booktabs = T, col.names = c("Generation", "Birth Years"), align = c('l', 'r'))
```
...
```{r, echo=FALSE, warning=FALSE, fig.height=4, fig.width=9, fig.align='c'}
